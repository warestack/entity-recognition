...
import pytest
from entity_recognition_lib.functions.recommendation_generation import dynamic_score_entities
...
def test_dynamic_score_entities_consistency(mock_cosine_similarity, mock_get_embedding, tech_entities_fixture):
    ...
    user_input = (
        "We are comparing MySQL and MongoDB to find the best database system based on performance and features."
    )
...
def test_dynamic_score_entities_no_match(tech_entities_fixture):
    ...
    assert (
        len(results) == 1 and results[0]["category"] == "Uncategorized"
    ), "Unknown entities should be categorized as 'Uncategorized'"
